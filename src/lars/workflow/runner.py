...
async def run_turn(graph: Any, config: dict[str, Any], *, telegram_id: int, text: str) -> str:
    ...
    if snapshot.interrupts:
        result = await graph.ainvoke(Command(resume=text), config)
    else:
        result = await graph.ainvoke({"telegram_id": telegram_id, "text": text}, config)
